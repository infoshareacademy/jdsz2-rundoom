select count(1) as ilosc_wnioskow,date_part('quarter',data_utworzenia)as kwartal,extract(year from data_utworzenia)as rok,
...
count(case when stan_wniosku = 'zaakceptowany przez operatora' then id
 ...
 when stan_wniosku = 'zamkniety' and kwota_rekompensaty >0 then id
  ...
  from wnioski
group by date_part('quarter',data_utworzenia),extract(year from data_utworzenia)
